-- ============================================================================
-- DATABASE RUNTIME VERIFICATION SCRIPT (FINAL READ-ONLY VERIFICATION CANDIDATE)
-- ============================================================================
...
--     Use Part E (Query-Level Prerequisite Matrix) to confirm query readiness
...
-- ============================================================================
-- Purpose: Inspect active RLS policy enforcement by starting from pg_class tables
--          and LEFT JOINing pg_policies. Distinguishes RLS DISABLED vs NO POLICIES vs ACTIVE POLICY.

SELECT 'D.1 All Tables with RLS Enforcement Status & Defined Policies (pg_class Primary Join)' AS section;
SELECT
  c.relname AS table_name,
  c.relrowsecurity AS rls_enabled,
  c.relforcerowsecurity AS rls_forced,
  ...
  CASE
    WHEN c.relrowsecurity = false THEN 'RLS DISABLED (UNPROTECTED TABLE)'
    WHEN p.policyname IS NULL THEN 'RLS ENABLED — NO POLICIES DEFINED (DENY ALL NON-OWNER ACCESS)'
    ELSE 'RLS ENABLED — ACTIVE POLICY ENFORCED'
...
SELECT 'D.2 Participants Table - Unconditional Boolean TRUE Policy Check (P0-3 Risk)' AS section;
SELECT
  p.policyname, p.permissive, p.roles, p.cmd AS operation, p.qual AS using_expression, p.with_check AS with_check_expression,
  c.relrowsecurity AS rls_enabled,
  CASE
    WHEN c.relrowsecurity = false THEN 'POLICY DEFINED BUT RLS DISABLED (NOT ENFORCED)'
    WHEN 'public'::name = ANY(p.roles) OR 'anon'::name = ANY(p.roles) OR 'authenticated'::name = ANY(p.roles)
      THEN 'BROAD ROLE UNCONDITIONAL TRUE BYPASS (P0-3 CRITICAL RISK)'
    ELSE 'PRIVILEGED ROLE UNCONDITIONAL TRUE POLICY (REVIEW REQUIRED)'
...
SELECT 'D.3 Organisations Table - Unconditional Boolean TRUE Policy Check (P0-4 Risk)' AS section;
SELECT
  p.policyname, p.permissive, p.roles, p.cmd AS operation, p.qual AS using_expression, p.with_check AS with_check_expression,
  c.relrowsecurity AS rls_enabled,
  CASE
    WHEN c.relrowsecurity = false THEN 'POLICY DEFINED BUT RLS DISABLED (NOT ENFORCED)'
    WHEN 'public'::name = ANY(p.roles) OR 'anon'::name = ANY(p.roles) OR 'authenticated'::name = ANY(p.roles)
      THEN 'BROAD ROLE UNCONDITIONAL TRUE BYPASS (P0-4 CRITICAL RISK)'
    ELSE 'PRIVILEGED ROLE UNCONDITIONAL TRUE POLICY (REVIEW REQUIRED)'
  END AS risk_classification
FROM pg_policies p
JOIN pg_class c ON c.relname = p.tablename
...
SELECT 'D.4 Mark Entries Table - All Defined Policies Audit' AS section;
SELECT
  p.policyname, p.permissive, p.roles, p.cmd AS operation, p.qual AS using_expression, p.with_check AS with_check_expression,
  c.relrowsecurity AS rls_enabled
FROM pg_policies p
...
SELECT 'D.4a Mark Entries High-Risk Command-Aware Access Summary (P0-1 Risk)' AS section;
SELECT
  p.policyname, p.roles, p.cmd AS operation, p.qual AS using_expression, p.with_check AS with_check_expression,
  c.relrowsecurity AS rls_enabled,
  'HIGH RISK: Unrestricted public/anon/authenticated mark entry operation' AS risk_description
FROM pg_policies p
JOIN pg_class c ON c.relname = p.tablename
JOIN pg_namespace n ON c.relnamespace = n.oid AND n.nspname = p.schemaname
WHERE p.schemaname = 'public' AND p.tablename = 'mark_entries'
...
AND (
  (p.cmd IN ('SELECT', 'DELETE') AND (qual = 'true' OR TRIM(BOTH '()' FROM TRIM(qual)) = 'true'))
  OR (p.cmd = 'INSERT' AND (with_check = 'true' OR TRIM(BOTH '()' FROM TRIM(with_check)) = 'true'))
  OR (p.cmd IN ('UPDATE', 'ALL') AND (
    qual = 'true' OR with_check = 'true'
    OR TRIM(BOTH '()' FROM TRIM(qual)) = 'true'
    OR TRIM(BOTH '()' FROM TRIM(with_check)) = 'true'
  ))
...
[
  {
    "policyname": "Super admin delete access to API keys",
  ...
  {
    "policyname": "Super admin insert access to API keys",
    "permissive": "PERMISSIVE",
    "roles": "{authenticated}",
    "operation": "INSERT",
  ...
  {
    "policyname": "Super admin read access to API keys",
    "permissive": "PERMISSIVE",
    "roles": "{authenticated}",
    "operation": "SELECT",
    "using_expression": "is_superadmin()",
  ...
  {
    "policyname": "Super admin update access to API keys",
    "permissive": "PERMISSIVE",
    "roles": "{authenticated}",
    "operation": "UPDATE",
    "using_expression": "is_superadmin()",
...
SELECT 'D.7 Policy Role References Requiring Runtime Validation (P0-8 Neutral Audit)' AS section;
...
WITH query_prerequisites (query_id, target_section, required_tables, required_columns) AS (
  VALUES
    ...
    ('F.33', 'F.33 Relational Dangling References', ARRAY['registrations', 'participants', 'results', 'mark_entries'], ARRAY['registrations.id', 'registrations.participant_id', 'participants.id', 'results.registration_id', 'mark_entries.registration_id']),
    ...
    ('F.35', 'F.35 Multi-Festival Active Readiness Report', ARRAY['tenants', 'festival_calendar'], ARRAY['tenants.id', 'tenants.is_active', 'festival_calendar.tenant_id', 'festival_calendar.id', 'festival_calendar.is_active']),
    ('F.36', 'F.36 Cross-Boundary Competition Integrity', ARRAY['registrations', 'participants', 'items', 'results', 'schedules', 'judges', 'judge_tokens', 'mark_entries'], ARRAY['registrations.id', 'registrations.participant_id', 'registrations.item_id', 'participants.id', 'participants.festival_id', 'items.id', 'items.festival_id', 'results.registration_id', 'results.festival_id', 'schedules.id', 'schedules.festival_id', 'judges.id', 'judges.festival_id', 'judge_tokens.schedule_id', 'judge_tokens.judge_id', 'mark_entries.judge_id', 'mark_entries.schedule_id', 'mark_entries.registration_id']),
