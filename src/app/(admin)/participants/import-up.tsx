...
// Mapping dictionary for Upper Primary events
const UP_EVENT_MAPPING: Record<string, string> = {
  'MAPPILA PATTU': 'UP-001',
  'STORYTELLING': 'UP-002',
  'ELOCUTION': 'UP-003',
  'MATH GAMES': 'UP-004',
  'QUIZ': 'UP-005',
  'PENCIL DRAWING': 'UP-006',
  'WATERCOLOR PAINTING WATERCOLORS': 'UP-007',
  'STORY WRITING': 'UP-008',
  'BOOK TEST': 'UP-009',
  'SPELLING BEE': 'UP-010',
  'SUDOKU': 'UP-011',
  'PENCIL DRAWING (GIRLS ONLY)': 'UP-012',
  'WATERCOLOR PAINTING (GIRLS ONLY)': 'UP-013',
  'BOOK TEST (GIRLS)': 'UP-014',
  'STORY WRITING (GIRLS ONLY)': 'UP-015',
  'ORIGAMI (GIRLS ONLY)': 'UP-016',
};
...
export default function ImportUpperPrimaryDataset() {
  ...
  const autoLoadFile = async () => {
    try {
      const response = await fetch('/importjson/upbulck.json');
      if (response.ok) {
    ...
    try {
      ...
      const normalized = rawParticipants.map((p: any) => {
        ...
        evs.forEach((ev: any) => {
          ...
          const code = UP_EVENT_MAPPING[eName];
          if (code) {
  ...
  const executeImport = async () => {
    ...
    try {
      ...
      for (let i = 0; i < chunks.length; i++) {
        ...
        const { data: result, error: chunkErr } = await db.executeUpperPrimaryImportChunk({
          tenant_id: tenantId,
          festival_id: festivalId,
  ...
  return (
    <ScrollView className="flex-1 bg-ssf-bg py-6 px-4">
      <View className="flex-row items-center mb-6">
        ...
        <Text className="text-3xl font-poppins-black text-ssf-text">Upper Primary Import</Text>
      </View>

      <SsfCard className="mb-6 bg-blue-50 border border-blue-200">
        <Text className="font-poppins-bold text-blue-800 mb-2">Upper Primary Rules</Text>
      ...
      {dataset.length === 0 ? (
        <SsfCard className="p-8 items-center border-dashed border-2 border-ssf-border mb-6">
          ...
          <Text className="font-poppins-bold text-lg text-ssf-text mb-2">Upload upbulck.json</Text>
          <Text className="font-poppins text-xs text-ssf-text-muted text-center mb-6 max-w-xs">
            Select the production file at:{'\n'}
            <Text className="font-poppins-bold">web-for-sahi--main/importjson/upbulck.json</Text>
          </Text>
        ...
        <>
          ...
          <SsfCard className="mb-6">
            ...
            <View className="flex-row gap-4 mt-6">
              ...
              />
              <SsfButton 
                label={isProcessing ? `${progress}% Processing...` : "Run UP Import"} 
                className="flex-1" 
