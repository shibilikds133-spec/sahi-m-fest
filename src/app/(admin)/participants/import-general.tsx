...
// Dynamic event mapping from Supabase (GENERAL, CAT-A, CAT-B)
let EVENT_MAPPING: Record<string, string> = {};
...
export default function ImportUpperPrimaryDataset() {
  ...
  const autoLoadFile = async () => {
    try {
      const response = await fetch('/importjson/generalbulk.json');
      if (response.ok) {
  ...
  const processFileContent = (text: string) => {
    try {
      ...
      const EVENT_MAPPING: Record<string, string> = {
        ...
        'COLLAGE': 'GN-017',
        'DUFF': 'GN-002',
        'FAMILY MAGAZINE': 'GN-019',
        'MALAPPATTU': 'GN-010',
        'MAPPILA SONG WRITING': 'GN-014',
        'MOULID RECITATION': 'GN-006',
        'NASHEED': 'GN-018',
        'PROJECT': 'GN-016',
        'QASEEDA BURDA': 'GN-007',
        'QAWWALI': 'GN-012',
        'REVOLUTIONARY SONG': 'GN-008',
        'REVOLUTIONARY SONG WRITING': 'GN-013',
        'RISALA QUIZ': 'GN-011',
        'SPOT MAGAZINE': 'GN-001',
        'WALL PAINTING': 'GN-009',
        'GROUP SONG A': 'GN-004',
        'GROUP SONG B': 'GN-005'
      };
  ...
  const checkDbConflicts = async (data: UpParticipant[]) => {
    try {
      // Fetch all participants for fast memory-based resolution
      const { data: allParticipants, error: pErr } = await supabase
        .from('participants')
        .select('id, chest_number, name')
      ...
      if (allParticipants) {
        allParticipants.forEach(p => {
          if (p.chest_number) chestMap.set(p.chest_number.toString(), p.id);
        });
      }
      ...
      data.forEach(p => {
        let rawMembers = [p.name];
        let repName = p.name;
        if (p.name.includes(',')) {
          rawMembers = p.name.split(',').map(n => n.trim());
          const leader = rawMembers.find(n => n.toLowerCase().includes('(leader)'));
          if (leader) {
            repName = leader.replace(/\(Leader\)/gi, '').replace(/\(leader\)/gi, '').trim();
          } else {
            repName = rawMembers[0].trim();
          }
        }

        const chestNoStr = (p.chest_number || p.chest_no || '').toString();
        
        let participantId = null;
        if (chestNoStr && chestMap.has(chestNoStr)) {
          participantId = chestMap.get(chestNoStr);
          reused++;
          // Indicate that it matched existing
          conflicts.set(chestNoStr, { name: repName, isMismatch: false });
        } else {
          created++;
          conflicts.set(chestNoStr || `auto-${created}`, { name: repName, isMismatch: true }); // True implies new creation for UI display
        }

        // Attach resolved data so it can be used in executeImport
        (p as any).resolved_participant_id = participantId;
        (p as any).resolved_name = repName.toUpperCase();
        (p as any).raw_group_members = rawMembers.length > 1 ? JSON.stringify(rawMembers) : null;
      });

      setDbConflicts(conflicts);
      setIsReady(true);

      setHeaderMismatch(`DRY RUN PREVIEW: Reusing ${reused} existing participants. Creating ${created} new participants.`);
  ...
  const executeImport = async () => {
    ...
    try {
      ...
      let totalCreatedParts = 0;
      let totalReusedParts = 0;
      ...
      for (let i = 0; i < chunks.length; i++) {
        ...
        const { data: result, error: chunkErr } = await db.executeGeneralImportChunk({
          tenant_id: tenantId,
          festival_id: festivalId,
          session_id: sessionId,
          participants: chunk.map((p: any) => ({ 
            participant_id: p.resolved_participant_id,
            name: p.resolved_name,
            chest_no: p.chest_number || p.chest_no,
            category_code: p.category || 'GENERAL',
            gender: 'Boys', // Or from p.gender if provided
            events: (p.events || []).map((e: any) => ({
              item_code: e.item_code || e.event_name, // Use the mapped code!
              general_division: p.category || 'GENERAL',
              raw_group_members: p.raw_group_members
            }))
          }))
        });
        ...
        if (result) {
          totalCreatedParts += result.created_participants || 0;
          totalReusedParts += result.reused_participants || 0;
          totalImportedParts += (result.created_participants || 0) + (result.reused_participants || 0);
          ...
          if (result.unmapped_events) allInvalidItems = allInvalidItems.concat(result.unmapped_events);
      ...
      const finalReport = {
        imported_participants: totalCreatedParts,
        reused_participants: totalReusedParts,
  ...
  return (
    <ScrollView className="flex-1 bg-ssf-bg py-6 px-4">
      ...
      <SsfCard className="mb-6 bg-blue-50 border border-blue-200">
        <Text className="font-poppins-bold text-blue-800 mb-2">General Rules</Text>
      ...
      {dataset.length === 0 ? (
        <SsfCard className="p-8 items-center border-dashed border-2 border-ssf-border mb-6">
          ...
          <Text className="font-poppins text-xs text-ssf-text-muted text-center mb-6 max-w-xs">
            ...
            <Text className="font-poppins-bold">web-for-sahi--main/importjson/generalbulk.json</Text>
          </Text>
        ...
        <>
          ...
          <SsfCard className="mb-6">
              ...
              <View className="mt-2">
                ...
                {conflictsList.map(c => (
                  <View key={c.chest} className={`flex-row justify-between p-2 mb-1 rounded ${c.isMismatch ? 'bg-red-100 border border-red-200' : 'bg-green-50 border border-green-200'}`}>
                    ...
                    <Text className={`font-poppins text-xs ${c.isMismatch ? 'text-blue-700 font-poppins-bold' : 'text-green-700'}`}>
                      {c.isMismatch ? `Will Create New Participant` : `Matches Existing (Will Reuse)`}
                    </Text>
                  </View>
                ))}
              </View>
            ...
            <View className="flex-row gap-4 mt-6">
              ...
              />
              <SsfButton 
                label={isProcessing ? `${progress}% Processing...` : "Run General Import"} 
                className="flex-1" 
      ...
      {importReport ? (
        <SsfCard className="mb-6">
          ...
          <View style={{ gap: 12 }}>
            ...
            <View className="flex-row justify-between border-b border-ssf-border pb-2 bg-blue-50 p-2 rounded">
              <Text className="font-poppins text-sm text-blue-700">Reused Existing Participants:</Text>
              <Text className="font-poppins-bold text-sm text-blue-800">{importReport.reused_participants}</Text>
            </View>
