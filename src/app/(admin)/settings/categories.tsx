...
export default function CollegeFestCategoriesScreen() {
  ...
  const { data = [], isLoading, error, create, update, setActive } = useFestivalCategories(festival?.id);
  ...
  if (festival.festival_template !== 'college_fest') {
    return <View className="flex-1 items-center justify-center p-6"><Text>Custom categories are available only for College Fest.</Text></View>;
  }
  ...
  return (
    <ScrollView className="flex-1 bg-ssf-bg" contentContainerStyle={{ padding: 20, paddingBottom: 80 }}>
      ...
      <Text className="font-poppins-bold text-2xl text-ssf-text">College Fest Categories</Text>
      <Text className="mb-5 mt-1 font-poppins text-sm text-ssf-text-muted">Manual categories for this festival only.</Text>
