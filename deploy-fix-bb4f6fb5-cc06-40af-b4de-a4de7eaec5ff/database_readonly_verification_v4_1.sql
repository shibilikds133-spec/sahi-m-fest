-- ============================================================================
-- DATABASE RUNTIME VERIFICATION SCRIPT V4.1 (READ-ONLY)
-- ============================================================================
...
--   - Part A, Part C (C.1-C.8), Part D, Part E, and Part H are universally safe
--     catalog queries that execute on any PostgreSQL database.
--   - Part B, Part F, and Part G are schema-dependent data queries.
--     Use Part E (Prerequisite Matrix) to confirm table/column/type existence
...
--   - Main Sections (Parts A through H) return counts, boolean flags, catalog
--     names, and aggregate statistics. They NEVER return PII or raw secrets.
--   - OPTIONAL Subsections (B.5, C.9, C.10, F.35) return detailed code, DDL, or
--     record keys. REVIEW AND REDACT output BEFORE sharing publicly.
-- ============================================================================
...
-- PREREQUISITE: Run B.2, B.3, B.4 ONLY IF B.1 returns true
...
SELECT 'B.3 Applied Migration Metadata Summary (Safe Metadata - NO Raw Statements Text)' AS section;
SELECT
  version,
  COALESCE(name, 'N/A') AS migration_name,
  COALESCE(executed_at::text, inserted_at::text, 'N/A') AS executed_timestamp,
  CASE
    WHEN EXISTS (
      SELECT 1 FROM information_schema.columns
      WHERE table_schema = 'supabase_migrations' AND table_name = 'schema_migrations' AND column_name = 'statements'
    ) THEN COALESCE(array_length(statements, 1), 0)
    ELSE 0
...
-- ============================================================================
-- OPTIONAL SUBSECTION B.5: FORENSIC RAW MIGRATION SQL DUMP
-- WARNING: Raw migration statements may contain legacy seed data, initial test
-- credentials, or private configuration. REVIEW AND REDACT BEFORE SHARING.
-- ============================================================================

-- B.5: Raw Migration Statements Dump (Optional)
SELECT
  version,
  name,
  statements
...
-- ============================================================================
-- Purpose: Inspect pg_policies for static security vulnerabilities (P0-1 to P0-8).
...
SELECT 'D.2 Participants Table - Permissive Boolean TRUE Check (P0-3 Risk)' AS section;
...
SELECT 'D.3 Organisations Table - Permissive Boolean TRUE Check (P0-4 Risk)' AS section;
SELECT policyname, permissive, roles, cmd AS operation, qual AS using_expression, with_check AS with_check_expression
FROM pg_policies
...
SELECT 'D.8 Broad Authenticated Boolean TRUE Policies Audit (Type-Safe Role Inspection)' AS section;
SELECT tablename, policyname, roles, cmd AS operation, qual AS using_expression, with_check AS with_check_expression
FROM pg_policies
WHERE schemaname = 'public'
...
-- ============================================================================
-- Purpose: Pure catalog query producing a diagnostic matrix of expected tables,
--          required columns, and data types. Use output to determine which queries
--          in Part F and Part G can be safely run. Always safe to run.
...
SELECT
  ...
  COALESCE(
    array_agg(c.column_name || ':' || c.data_type ORDER BY c.ordinal_position) FILTER (WHERE c.column_name IS NOT NULL),
    ARRAY[]::text[]
  ) AS existing_columns_with_types,
  (
    SELECT array_agg(rc)
...
-- PREREQUISITE: Run only if 'registrations' and 'participants' exist (see Part E matrix)
SELECT 'F.30 Boundary Integrity - Registrations Tenant Mismatch with Participant Tenant' AS section;
SELECT COUNT(*)::bigint AS tenant_mismatch_count
FROM public.registrations r
JOIN public.participants p ON r.participant_id = p.id
WHERE r.tenant_id != p.tenant_id;

-- PREREQUISITE: Run only if 'results', 'registrations', 'participants' exist (see Part E matrix)
SELECT 'F.31 Boundary Integrity - Results Festival Mismatch with Participant Festival' AS section;
...
JOIN public.participants p ON r.participant_id = p.id
WHERE res.festival_id != p.festival_id;

-- PREREQUISITE: Run only if 'schedules' and 'items' exist (see Part E matrix)
SELECT 'F.32 Boundary Integrity - Schedules Festival Mismatch with Item Festival' AS section;
SELECT COUNT(*)::bigint AS schedule_item_festival_mismatch_count
FROM public.schedules s
JOIN public.items i ON s.item_id = i.id
WHERE s.festival_id != i.festival_id;

-- PREREQUISITE: Run only if 'participants', 'tenants', 'festival_calendar' exist (see Part E matrix)
SELECT 'F.33 Boundary Integrity - Participants Referencing Missing Tenant or Festival' AS section;
SELECT
  (SELECT COUNT(*)::bigint FROM public.participants p WHERE NOT EXISTS (SELECT 1 FROM public.tenants t WHERE t.id = p.tenant_id)) AS orphan_tenant_participants,
  (SELECT COUNT(*)::bigint FROM public.participants p WHERE p.festival_id IS NOT NULL AND NOT EXISTS (SELECT 1 FROM public.festival_calendar f WHERE f.id = p.festival_id)) AS orphan_festival_participants;

-- PREREQUISITE: Run only if 'festival_calendar' exists (see Part E matrix)
SELECT 'F.34 Boundary Integrity - Active Festival Count Anomalies Per Tenant' AS section;
SELECT
  COUNT(*) FILTER (WHERE active_count = 0)::bigint AS tenants_with_zero_active_festivals,
  COUNT(*) FILTER (WHERE active_count > 1)::bigint AS tenants_with_multiple_active_festivals
FROM (
  SELECT t.id AS tenant_id, COUNT(f.id) FILTER (WHERE f.is_active = true) AS active_count
...
-- ============================================================================
-- OPTIONAL SUBSECTION F.35: DETAILED FORENSIC DUPLICATE MARK ENTRIES
-- Purpose: Diagnostic query for remediation. Returns specific record keys.
-- ============================================================================

-- F.35: Detailed Duplicate Mark Entry Key Triples (Optional - Top 50)
