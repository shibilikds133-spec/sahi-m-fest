-- 109_fix_judge_token_registration_visibility.sql
-- Preserve token-bound schedule/festival and organisation-tree isolation while
-- accepting registrations whose participant organisation is the authoritative
-- tree member even when registrations.organisation_id is stale or different.
...
BEGIN
  ...
  WHERE registration.item_id = v_schedule_item_id
    ...
    AND (
      registration.organisation_id IN (SELECT id FROM organisation_tree)
      OR participant.organisation_id IN (SELECT id FROM organisation_tree)
    )
