-- Keep judge registrations inside the schedule's festival and organisation tree.
...
AS $$
  WITH RECURSIVE schedule_context AS (
    SELECT
      ...
      s.festival_id,
      s.tenant_id,
      t.organisation_id AS root_organisation_id
  ...
  organisation_tree AS (
    ...
    WHERE
      o.id = context.root_organisation_id
      OR (
        context.root_organisation_id IS NULL
        AND o.tenant_id = context.tenant_id
      )
