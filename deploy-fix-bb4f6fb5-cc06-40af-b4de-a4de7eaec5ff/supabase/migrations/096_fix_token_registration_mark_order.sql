-- Fix the valid-token path introduced in migration 093. mark_entries has no
-- created_at column; use the durable update/submission timestamps instead.

BEGIN;

-- Migration 093 writes and updates this audit timestamp but omitted the column
-- definition. The default safely backfills existing rows and supports both new
-- inserts and draft updates through upsert_judge_mark.
...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1
    FROM public.schedule_judge_assignments assignment
  ...
  SELECT tenant.organisation_id
  INTO v_root_org_id
  FROM public.tenants tenant
  WHERE tenant.id = v_schedule_tenant_id;
  ...
  LEFT JOIN LATERAL (
    ...
    ORDER BY
      mark_entry.updated_at DESC NULLS LAST,
      mark_entry.submitted_at DESC NULLS LAST,
      mark_entry.id DESC
    LIMIT 1
  ) mark ON true
  WHERE registration.item_id = v_schedule_item_id
    ...
    ) IN (SELECT organisation_tree.id FROM organisation_tree)
...
-- Apply the same unambiguous organisation-tree reference to the authenticated
-- schedule-id overload retained for the admin mark-entry screen.
