-- ============================================================================
-- DATABASE RUNTIME VERIFICATION SCRIPT V4.2 (READ-ONLY)
-- ============================================================================
...
--   - OPTIONAL Subsections (B.5, C.6, C.7, F.36) return detailed code, DDL, or
...
SELECT 'B.3 Applied Migration Metadata Summary (JSONB Composite Row Safe Form - NO Raw Statements Text)' AS section;
SELECT
...
-- ============================================================================
-- OPTIONAL SUBSECTION C.6 & C.7: FULL FUNCTION & VIEW DEFINITIONS
...
-- C.6: All Public Functions (Full Definition)
SELECT
  p.proname AS function_name,
...
--          required columns, and existing data types for manual schema-drift review.
--          Always safe to run.

SELECT 'E.1 Prerequisite Matrix for Core and Optional Tables (with Column Data Types)' AS section;
...
-- PREREQUISITE: Run only if 'registrations', 'results', 'mark_entries' exist (see Part E matrix)
SELECT 'F.33 Boundary Integrity - Dangling Relational References' AS section;
SELECT
  (SELECT COUNT(*)::bigint FROM public.registrations r WHERE NOT EXISTS (SELECT 1 FROM public.participants p WHERE p.id = r.participant_id)) AS registrations_missing_participant,
  (SELECT COUNT(*)::bigint FROM public.results res WHERE NOT EXISTS (SELECT 1 FROM public.registrations r WHERE r.id = res.registration_id)) AS results_missing_registration,
  (SELECT COUNT(*)::bigint FROM public.mark_entries me WHERE NOT EXISTS (SELECT 1 FROM public.registrations r WHERE r.id = me.registration_id)) AS marks_missing_registration;

-- PREREQUISITE: Run only if 'participants', 'tenants', 'festival_calendar' exist (see Part E matrix)
SELECT 'F.34 Boundary Integrity - Participants Ownership Analysis (Null vs Dangling Split)' AS section;
SELECT
...
-- PREREQUISITE: Run only if tables 'tenants' AND 'festival_calendar' exist (see Part E matrix)
SELECT 'F.35 Boundary Integrity - Active Festival Count Anomalies Per Tenant' AS section;
SELECT
...
-- ============================================================================
-- OPTIONAL SUBSECTION F.36: DETAILED FORENSIC DUPLICATE MARK ENTRIES
-- Purpose: Diagnostic query for remediation. Returns specific record keys.
-- ============================================================================

-- F.36: Detailed Duplicate Mark Entry Key Triples (Optional - Top 50)
