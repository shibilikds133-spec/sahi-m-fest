-- Migration 162: Allow team leaders to unassign items from participants
-- Relaxing the role restriction to include 'team_leader'.
-- The subsequent 'is_org_visible' check safely limits them to their own unit's participants.
...
BEGIN
  ...
  IF NOT FOUND OR (COALESCE(v_profile.is_superadmin, false) IS FALSE
      AND v_profile.role NOT IN ('admin', 'admin_leader', 'team_leader')) THEN
    RAISE EXCEPTION 'Only festival administrators or team leaders can unassign competition items.'
      USING ERRCODE = '42501';
  END IF;
