-- Migration 174: Align preview RPC with public RPC when p_use_public_only=true
-- When p_use_public_only is true, use the exact same result_status filter
-- as get_public_leaderboard (only 'published', not 'ready').

CREATE OR REPLACE FUNCTION public.get_leaderboard_preview(
  p_tenant_id uuid,
  p_festival_id uuid,
  p_ranking_mode text,
...
BEGIN
  ...
  WITH limited_items AS (
    ...
    WHERE festival_id = v_festival_id
      ...
      AND (
        CASE WHEN p_use_public_only
          THEN COALESCE(result_status, 'published') = 'published'
          ELSE COALESCE(result_status, 'draft') IN ('published', 'ready')
        END
      )
  ...
  deduped_results AS (
    ...
    WHERE res.festival_id = v_festival_id
      ...
      AND (
        CASE WHEN p_use_public_only
          THEN COALESCE(res.result_status, 'published') = 'published'
          ELSE COALESCE(res.result_status, 'draft') IN ('published', 'ready')
        END
      )
...
GRANT EXECUTE ON FUNCTION public.get_leaderboard_preview(uuid, uuid, text, integer, boolean) TO authenticated;
