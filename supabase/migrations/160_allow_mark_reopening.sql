-- Fix trigger to allow unlocking (reopening) finalized marks.
...
BEGIN
  IF TG_OP = 'UPDATE'
     ...
     AND (
       ...
       -- Allowed: NEW.is_final IS DISTINCT FROM OLD.is_final (for unlocking)
       OR NEW.is_draft IS DISTINCT FROM OLD.is_draft
       OR NEW.submitted_at IS DISTINCT FROM OLD.submitted_at
     )
  THEN
    RAISE EXCEPTION 'Final marks cannot be modified directly. They must be unlocked first.';
  END IF;
