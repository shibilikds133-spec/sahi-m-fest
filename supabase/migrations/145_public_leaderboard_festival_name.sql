-- Public leaderboard festival name
-- The admin-controlled label is scoped to the existing festival leaderboard
-- settings row. Empty values fall back to festival_calendar.custom_name.

ALTER TABLE public.festival_leaderboard_settings
  ADD COLUMN IF NOT EXISTS public_festival_name text DEFAULT NULL;
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(
      NULLIF(trim(settings.public_festival_name), ''),
      NULLIF(trim(fc.custom_name), ''),
      'Festival'
    ) AS public_festival_name
...
GRANT EXECUTE ON FUNCTION public.get_public_leaderboard_settings(uuid, uuid)
