-- ============================================================================
-- DATABASE RUNTIME VERIFICATION SCRIPT V2 (READ-ONLY)
-- ============================================================================
...
--          This section is robust against missing migration table.
...
-- B.2: Only execute if table exists
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.tables
    ...
    RAISE NOTICE 'Migration table found';
  ELSE
    RAISE NOTICE 'Migration table NOT found - Supabase migrations may not be configured';
  END IF;
END $$;

-- B.3: Query migration history (will error if table missing, but won't stop script)
SELECT 'B.3 Applied Migrations' AS section;
SELECT version, name, statements, executed_at
FROM supabase_migrations.schema_migrations
ORDER BY version;

SELECT 'B.4 Migration Count' AS section;
...
-- D.8: System API Keys - existence and policies (guarded)
SELECT 'D.8 System API Keys - Existence and Policies' AS section;
SELECT
  EXISTS (
...
-- D.9: System API Keys policies (only if table exists)
SELECT
  p.policyname,
  p.roles,
  p.cmd AS operation,
...
SELECT 'E.1 All Public Functions (Full Definition)' AS section;
SELECT
...
-- E.3: Function privileges for anon and authenticated roles
-- NOTE: PostgreSQL PUBLIC is a pseudo-role. We check anon and authenticated
-- separately, then check PUBLIC via has_function_privilege with role name.
SELECT 'E.3 Function Privileges - Anon and Authenticated' AS section;
SELECT
  p.proname AS function_name,
...
-- E.8: All views
SELECT 'E.8 All Views' AS section;
SELECT
  viewname,
  definition
FROM pg_views
...
--          Each subsection guards against missing tables and columns.

-- G.1: System API Keys (created by root SQL, may not be in migrations)
SELECT 'G.1 System API Keys - Module Check' AS section;
SELECT
  EXISTS (
    SELECT 1 FROM pg_tables
    WHERE schemaname = 'public' AND tablename = 'system_api_keys'
...
-- G.2: System API Keys - row count and provider distribution (guarded)
SELECT 'G.2 System API Keys - Data' AS section;
SELECT
  provider,
  COUNT(*) AS key_count,
  COUNT(*) FILTER (WHERE is_active = true) AS active_count
...
SELECT 'G.3 File Metadata - Module Check' AS section;
SELECT
  EXISTS (
    SELECT 1 FROM pg_tables
...
-- G.4: File Metadata - stats (guarded)
SELECT 'G.4 File Metadata - Stats' AS section;
SELECT
  asset_type,
  COUNT(*) AS count,
...
-- G.5: Notifications (created in migration 074)
SELECT 'G.5 Notifications - Module Check' AS section;
SELECT
  EXISTS (
    SELECT 1 FROM pg_tables
...
-- G.6: Notifications - stats (guarded)
SELECT 'G.6 Notifications - Stats' AS section;
SELECT
  COUNT(*) AS total_notifications,
  COUNT(DISTINCT tenant_id) AS tenant_count
FROM public.notifications;

-- G.7: Notification Logs - stats (guarded)
SELECT 'G.7 Notification Logs - Stats' AS section;
SELECT
  status,
  COUNT(*) AS count
...
-- G.8: User Notification Tokens - count (guarded)
SELECT 'G.8 User Notification Tokens - Count' AS section;
SELECT
  COUNT(*) AS total_tokens,
  COUNT(DISTINCT user_id) AS unique_users
FROM public.user_notification_tokens;

-- G.9: Audit Logs - count (guarded)
SELECT 'G.9 Audit Logs - Count' AS section;
SELECT COUNT(*) AS total_audit_logs FROM public.audit_logs;

-- G.10: System Events - count (guarded)
SELECT 'G.10 System Events - Count' AS section;
SELECT COUNT(*) AS total_events FROM public.system_events;

-- G.11: Participant Unit Batches - count (guarded)
SELECT 'G.11 Participant Unit Batches - Count' AS section;
SELECT COUNT(*) AS total_batches FROM public.participant_unit_batches;

-- G.12: Participant Unit Audit Logs - count (guarded)
SELECT 'G.12 Participant Unit Audit Logs - Count' AS section;
SELECT COUNT(*) AS total_audit_logs FROM public.participant_unit_audit_logs;

-- G.13: Import Sessions - count (guarded)
SELECT 'G.13 Import Sessions - Count' AS section;
SELECT COUNT(*) AS total_sessions FROM public.import_sessions;

-- G.14: Poster Templates - count (guarded)
SELECT 'G.14 Poster Templates - Count' AS section;
SELECT COUNT(*) AS total_templates FROM public.poster_templates;

-- G.15: Poster Drafts - count (guarded)
SELECT 'G.15 Poster Drafts - Count' AS section;
SELECT COUNT(*) AS total_drafts FROM public.poster_drafts;

-- G.16: Poster Versions - count (guarded)
SELECT 'G.16 Poster Versions - Count' AS section;
SELECT COUNT(*) AS total_versions FROM public.poster_versions;

-- G.17: Generated Posters - count (guarded)
SELECT 'G.17 Generated Posters - Count' AS section;
SELECT COUNT(*) AS total_generated FROM public.generated_posters;

-- G.18: Generated Assets - existence check
SELECT 'G.18 Generated Assets - Module Check' AS section;
SELECT
  EXISTS (
    SELECT 1 FROM pg_tables
...
-- G.19: Export Jobs - existence check
SELECT 'G.19 Export Jobs - Module Check' AS section;
SELECT
  EXISTS (
    SELECT 1 FROM pg_tables
