...
async function debugPoints() {
  ...
  for (const res of results) {
    ...
    const newPtsObj = typeof calculatedPoints === 'string' ? JSON.parse(calculatedPoints) : calculatedPoints;
    const finalTotal = newPtsObj ? newPtsObj.total : 0;
    
    const newPts = finalTotal ? parseFloat(finalTotal).toFixed(2) : '0.00';
    ...
    if (newPts !== oldPts) {
      ...
      const { error: updateError } = await supabase
        .from('results')
        .update({ points_awarded: finalTotal })
        .eq('id', res.id);
