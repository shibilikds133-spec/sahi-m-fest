...
const startJsx = `              {results.map((res, index) => {
                const isTop = index === 0;
                return (
...
const endJsx = `                  </TouchableOpacity>
                </View>
              );
            })}`;

const startJsxIndex = code.indexOf(startJsx);
const endJsxIndex = code.indexOf(endJsx) + endJsx.length;
if (startJsxIndex !== -1 && endJsxIndex !== -1) {
  code = code.substring(0, startJsxIndex) + replacementJsx + `
                );
              })}` + code.substring(endJsxIndex);
} else {
  console.log("JSX NOT FOUND");
}

// Replace the styles
const stylesStart = `  card: {`;
const stylesEnd = `  publishText: {
    color: 'white',
    fontWeight: '600',
    fontSize: 16,
  },`;
...
const stStart = code.indexOf(stylesStart);
const stEnd = code.indexOf(stylesEnd) + stylesEnd.length;
if (stStart !== -1 && stEnd !== -1) {
  code = code.substring(0, stStart) + stylesReplacement + code.substring(stEnd);
} else {
