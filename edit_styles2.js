...
const startJsx = code.indexOf("{results.map((res, index) => {");
const endJsx = code.indexOf("})}") + 3;
...
if (startJsx !== -1 && endJsx !== -1) {
  ...
  code = code.substring(0, startJsx) + replacementJsx + code.substring(endJsx);
  fs.writeFileSync('src/app/(admin)/smart-announcer/index.tsx', code, 'utf-8');
  console.log("JSX replaced.");
} else {
  console.log("JSX NOT FOUND", startJsx, endJsx);
}
